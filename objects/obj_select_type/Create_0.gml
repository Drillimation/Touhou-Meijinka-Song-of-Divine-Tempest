...
if struct_exists(_load_data, "types") {
	var _names = struct_get_names(_load_data.types);
	for(var i = 0; i < array_length(_names); i += 1) {
		button[i] = string(struct_get(_load_data.types,"a" + string(i)));
	}
}

if struct_exists(_load_data, "type_desc") {
	var _names = struct_get_names(_load_data.type_desc);
	for(var i = 0; i < array_length(_names); i += 1) {
